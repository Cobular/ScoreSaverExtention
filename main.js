...
function showButton() {
  ...
  downloadButtonElement.innerHTML = `
        ...
        <div style="width: 64px">
            ...
            <a href="beatsaver://${key}" class="button tooltip saberSaverButton umami--click--download-button-${key}"><img src="${imageUrl}" alt="Download Button" title="Download Button">
                  <span class="tooltiptext">${chrome.i18n.getMessage("button_text")}</span>
            </a>
        </div>
    `;

  document.querySelector(".column .is-one-quarter").appendChild(downloadButtonElement)
}

let song_hash = document.querySelector("div.has-shadow > b").textContent;
